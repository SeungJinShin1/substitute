...
const AssignmentCenter = ({ requestData, teachers, updateTeacherCount }) => {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {loading ? (
          <div className="bg-white p-12 rounded-2xl flex flex-col items-center justify-center text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mb-4"></div>
            <h3 className="text-xl font-bold text-indigo-900">AI 분석 중...</h3>
            <p className="text-gray-500">학교 규정 및 NEIS 시간표 연동(예정) 데이터를 분석합니다.</p>
          </div>
        ) : result ? (
          <div className="space-y-4 animate-fade-in">
            {result.assignments.map((item, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:border-indigo-200">
                <div className="flex items-start gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-100 text-indigo-600 flex flex-col items-center justify-center font-bold">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-md">누적 {item.currentCount}회</span>
                    </div>
            ...
            <div className="flex justify-end">
              <button onClick={confirmAssignment} className="bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg hover:bg-indigo-700 flex items-center gap-2">
                <CheckCircle2 /> 배정 확정하기
              </button>
            </div>
          </div>
        ) : null}
